...
const std::unordered_map<uint32_t, size_t> kParameterSizeMap = {
...
void GlSampler::commit() {
  for (auto& [pname, param] : this->pending_.params_) {
    this->current_.params_.insert_or_assign(pname, param);
  }
  this->pending_.params_.clear();
}
...
void GlSampler::sync(bool force_sync) {
  ...
  for (auto& [pname, param] : this->current_.params_) {
    ...
    switch (param.type_) {
      ...
      case ParamType::I: {
        auto value = std::get<int32_t>(param.param_);
        this->proxy_->get()->GlSamplerParameteri(pname, value);
        break;
      }
      case ParamType::FV: {
        auto& value = std::get<std::array<uint8_t, 16>>(param.param_);
...
void GlSampler::set_paramater(wl_resource* resource, ParamType type,
  ...
  {  // valiation block
    ...
    try {
      expected_size = kParameterSizeMap.at(pname);
    ...
    if (std::holds_alternative<wl_array*>(param)) {
      ...
      if (array->size != expected_size) {
        kSizeErr(expected_size, array->size);
        return;
      }
    } else {
      ...
      if (actual_size != expected_size) {
        kSizeErr(expected_size, actual_size);
        return;
      }
    }
  }
